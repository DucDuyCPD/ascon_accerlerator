...
def demo_write_to_file(data, filename="output.txt"):
    maxlen = max([len(text) for (text, val) in data])
    with open(filename, "w", encoding="utf-8") as file:
        for text, val in data:
            file.write("{text} {val} {length}\n".format(text=text, align=((maxlen - len(text)) * " "), val=bytes_to_hex(val), length=len(val)))
...
def demo_aead(variant="Ascon-AEAD128"):
    ...
    associateddata = get_random_bytes(512*1024) #62 bytes
    plaintext      = get_random_bytes(512*1024)
...
def demo_hash(variant="Ascon-Hash256", hashlength=32):
    ...
    message = get_random_bytes(1024*1024)
